;(function ($, window, document) {
    $(document).ready(function() {
		...
		$(".myTradeListMenu").click(function(){
             // show Modal
             $('#tradeListModal').modal('show');
        });
...
function CheckDrops() {
    $.ajax({
        ...
        url: '/countdrops.php',
        success: function(data) {
			$(".itemCount").text(droparray.length);
            if (data == 0) {
                $("#dropCounts").html('');
				$(".dropCount").text('0');
            } else {
                $("#dropCounts").html('<div class="diablo" style="margin-left:25px;margin-top:5px;font-weight:bold;text-align:center;">Dropping ' + data + ' item(s).</div>');
				$(".dropCount").text(data);
            }
        },
